 ...
 * (замер ~27 с) он отнесён к тегу `slow`, а весь `slowTest` идёт 8 мин 39 с и потому
 * в `check` НЕ входит (он гоняется job'ом `full` в CI). Без этой задачи главный
 * гейт не исполнялся бы локально вовсе — только в CI через восемь минут.
 ...
 * `characterizationTest`: весь `slowTest` стоит 8 мин 39 с и в локальный `check` не
 * укладывается (он гоняется job'ом `full` в CI). Именно эта задача, а не `slowTest`,
 * подключена к `check`.
 */
 ...
 * `characterizationTest`: весь `slowTest` стоит 8 мин 39 с и потому в `check` не входит,
 * а без своей задачи проверка порядков была бы исполнима только вручную либо
 * целиком вместе со всем slow-набором. Тег класса при этом не меняется: метод
 * полной матрицы остаётся `slow` и входит в `slowTest`.
 *
