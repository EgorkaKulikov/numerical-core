...
/**
 * Contract of [DenseMatrix] and agreement between the single-entry-point overloads on [DenseMatrix]
 * and those on [Array]<[DoubleArray]>: the adapters must produce bit-for-bit identical results.
 */
@Tag("fast")
class DenseMatrixTest {
    ...
    /** Symmetric positive definite A^T A + n·I. */
    ...
    private fun assertRowsBitwise(expected: Array<DoubleArray>, actual: DenseMatrix, what: String) {
        assertEquals(expected.size, actual.rows, "$what: row count")
        for (i in expected.indices) assertContentEquals(expected[i], actual.row(i), "$what: row $i")
    }
    ...
    @Test
    fun `mismatched array length is rejected`() {
    ...
    @Test
    fun `ragged fromRows is rejected naming the row`() {
        ...
        assertTrue(e.message!!.contains("Row 1 has length"), e.message)
    }

    @TestFactory
    fun `empty matrices`(): List<DynamicTest> = listOf(
        DynamicTest.dynamicTest("0x0 from an empty array") {
    ...
    @Test
    fun `identity diagonal build and column-major layout`() {
    ...
    @TestFactory
    fun `out-of-bounds access in get and set`(): List<DynamicTest> {
        val m = DenseMatrix.zeros(2, 3)
        return listOf(-1 to 0, 2 to 0, 0 to -1, 0 to 3, 2 to 3).map { (i, j) ->
            DynamicTest.dynamicTest("($i, $j) in 2x3") {
    ...
    @Test
    fun `set writes into data and copy is independent`() {
    ...
    @TestFactory
    fun `fromRows after toRows restores the matrix bit-for-bit`(): List<DynamicTest> = shapes.map { (r, c) ->
    ...
    @TestFactory
    fun `double transpose is the identity`(): List<DynamicTest> = shapes.map { (r, c) ->
        DynamicTest.dynamicTest("${r}x$c") {
    ...
    @Test
    fun `fromColumnMajor does not copy the array`() {
    ...
    // --- agreement with the single entry point --------------------------------------------------

    private val sizes = listOf(1, 2, 3, 5, 8, 16)

    @TestFactory
    fun `identity times A equals A via matMat`(): List<DynamicTest> = sizes.map { n ->
    ...
    @TestFactory
    fun `DenseMatrix overloads agree bit-for-bit with Array overloads`(): List<DynamicTest> = sizes.flatMap { n ->
    ...
    @TestFactory
    fun `assembleDense equals assembleMatrix bit-for-bit`(): List<DynamicTest> =
