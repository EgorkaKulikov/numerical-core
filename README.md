...
|---|---|
| `DenseMatrix` | Плотная матрица в столбцовом порядке BLAS/LAPACK; передаётся без переупорядочения, исходные данные не изменяются. Конструкторы `zeros`, `identity`, `diagonal`, `fromRows`, `fromColumnMajor`, `build`. |
...
Все операции `O(n³)` выполняет BLAS/LAPACK. Матрицы хранятся в столбцовом порядке и
передаются в нативную библиотеку без переупорядочения; копия снимается только там, где
LAPACK перезаписывает вход (решение систем, разложения), поэтому исходные матрицы не
изменяются. Потоки внутри операций линейной алгебры создаёт нативная библиотека и
управляет ими через свои переменные окружения (`OMP_NUM_THREADS`, `OPENBLAS_NUM_THREADS`,
...
Apache License 2.0. Правообладатель — Куликов Егор Константинович. Текст лицензии — в файле `LICENSE`,
