 ...
 *
 * Через этот интерфейс фасад [numerics.LinearAlgebra] обращается к вычислительному
 * ядру; реализация подбирается в [Backends] и передаётся фасаду явным параметром.
 ...
 *  - согласованность размерностей проверяет фасад, реализация вправе полагаться на
 *    неё и не обязана дублировать проверки;
 *  - результат всегда новый объект, не разделяющий память с входом.
 */
interface LinAlgBackend {

    /** Человекочитаемое имя реализации для логов, бенчмарков и диагностики. */
    val name: String
    ...
    fun matTransVec(a: DenseMatrix, y: DoubleArray): DoubleArray

    /** Произведение A·B для A размера m×k и B размера k×p; результат m×p. */
    fun matMat(a: DenseMatrix, b: DenseMatrix): DenseMatrix

    /** Произведение Aᵀ·B для A размера m×n и B размера m×p; результат n×p. */
    fun matTransMat(a: DenseMatrix, b: DenseMatrix): DenseMatrix
     ...
     */
    fun solve(a: DenseMatrix, b: DenseMatrix): DenseMatrix
     ...
     */
    fun luFactor(a: DenseMatrix): LuFactorization
     ...
     */
    fun cholesky(a: DenseMatrix): DenseMatrix?
     ...
     */
    fun reciprocalCondition1(lu: LuFactorization, norm1: Double): Double

    /** Матричная норма указанного вида; для пустой матрицы — 0. */
    fun norm(a: DenseMatrix, kind: MatrixNorm): Double
}
 ...
 */
class LuFactorization(val lu: DenseMatrix, val ipiv: IntArray, val singularAt: Int) {
    /** Истина, если в ходе разложения встретился нулевой ведущий элемент. */
    val isSingular: Boolean get() = singularAt > 0
}
